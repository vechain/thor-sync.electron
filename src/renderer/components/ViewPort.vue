      ...
      {{title}}
        ...
        <v-container fluid class='pa-0 ma-0 sync-viewport-bar'>
            <v-layout row>
                ...
                <v-flex>
                    <v-text-field class="custom-field mt-0" height="34px" v-model="origin" :placeholder="url" @keyup.enter="emitNewTab({ url: origin })"
                        background-color="white" prepend-inner-icon="search" single-line :hide-details="true"></v-text-field>
                </v-flex>
                <v-flex>
                </v-flex>
            </v-layout>
        </v-container>
        <webview v-if="url" :partition="partition" v-poster autosize :preload="preload" :src="url"></webview>
        <!-- <DApps @open-dapp="addPort" class="default-content" :list="apps"></DApps> -->
        <DApps @open-dapp="openDapp" v-if="!url" class="default-content" :list="apps">
            <!-- <div class="search">
